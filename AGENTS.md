...
- **Patches upstream source** (`portable_paths.py`). Rewrites `paths.py`, `loader.py`, `schema.py`, `cli/commands.py` (serve) + `cli/agent.py` (agent), `utils/helpers.py`, `agent/memory.py`. Idempotent — logs `[WARN] pattern not found` for misses or `[SKIP] already patched`. Since upstream v0.2.2+ serve/agent call `_set_nanobot_logs(<flag>)` (loguru enable/disable): nanowin replaces that call with its own handlers — terminal INFO (clean UX) or DEBUG with `--verbose`/`--logs`, file DEBUG to `logs/nanobot_{date}.log`. Every patch skips cleanly on re-run. Memory pin patch ensures `MemoryStore` always uses config workspace (`NANOBOT_HOME/workspace`) so memory survives WebUI workspace scope changes.
